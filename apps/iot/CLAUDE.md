...
> Les règles transverses du monorepo (architecture, tests, git, sécurité) sont dans `.claude/commands/rules.md` (`/rules`), mais **la plupart ne s'appliquent pas ici** (C++ / embarqué). Retiens uniquement : pas de secret en git, conventional commits, review obligatoire, et tests là où c'est possible (`pio test` sur la logique pure).
