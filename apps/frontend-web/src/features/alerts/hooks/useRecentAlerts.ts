...
// déclenchement (desc) côté backend-central. Polling aligné sur la liste.
export function useRecentAlerts(): UseQueryResult<ConsolidatedResponse<Alert>> {
  return useQuery({
    queryKey: ['alerts', 'recent', RECENT_ALERTS_COUNT],
    queryFn: () =>
      fetchAlerts({ page: 1, pageSize: RECENT_ALERTS_COUNT }),
